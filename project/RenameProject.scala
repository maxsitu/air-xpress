...
object RenameProject {
  ...
  val renameArgsParser: Parser[RenameCommand] = {
    ...
    (Space ~>
      token(NotSpace, "top level package, for example com.jc") ~
      Space ~
      token(NotSpace, "project name, single word like jc")).map {
  ...
  val renameHelp = Help(
    ...
    Map(
      "renameProject" ->
        ...
        |<projectName> - project name, a single word like jetclub
      """.stripMargin
    )
  )
