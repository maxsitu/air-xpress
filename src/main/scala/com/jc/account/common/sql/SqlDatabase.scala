...
import com.jc.account.common.sql.DatabaseConfig._
...
object SqlDatabase extends StrictLogging {
  ...
  def createPostgresFromConfig(config: DatabaseConfig) = {
    val db = Database.forConfig("jc.account.db.postgres", config.rootConfig)
  ...
  private def createEmbedded(config: DatabaseConfig): SqlDatabase = {
    val db = Database.forConfig("jc.account.db.h2")
